    ...
    def setup(self):
        def _setup(self):
            ...
            if not os.path.isabs(self.options['certdir']):
                self.logger.dbg(
                    'Certificate directory path was not absolute. Assuming relative to current programs\'s directory')
            ...
            if not self.options['cacert']:
                ...
                if not os.path.isdir(self.options['cacert']):
                    ...
                    p = Popen(
                        ["openssl", "req", "-new", "-x509", "-days", "3650", "-key", self.options['cakey'], "-out",
                         self.options['cacert'], "-subj", "/CN=" + self.options['cacn']], stdout=PIPE, stderr=PIPE)
                    (out, error) = p.communicate()
                    self.logger.dbg(out + error)
